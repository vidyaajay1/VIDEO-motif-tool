...
def plot_overview_single_task(
    data_id: str,
    window: int,
    per_motif_pvals_json: str,
    want_download: bool,  # whether to produce CSV on disk
):
    _progress(1)
    # Load caches
    ...
    gene_lfc_fp  = os.path.join(TMP_DIR, f"{data_id}_genes_lfc.pkl")

    if not all(os.path.exists(p) for p in [peaks_fp, hits_fp, motifs_fp, gene_lfc_fp]):
        missing = [p for p in [peaks_fp, hits_fp, motifs_fp, gene_lfc_fp] if not os.path.exists(p)]
        raise RuntimeError(f"Missing cached inputs for {data_id}: {missing}")

    peaks_df  = pd.read_pickle(peaks_fp)
    df_hits   = pd.read_pickle(hits_fp)
    motiflist = pd.read_pickle(motifs_fp)
    gene_lfc  = pd.read_pickle(gene_lfc_fp)
    ...
    _progress(90)
    # Write artifacts
    _write_json(os.path.join(TMP_DIR, f"{data_id}_overview.json"), json.loads(fig.to_json()))
    _write_json(os.path.join(TMP_DIR, f"{data_id}_ordered_peaks.json"), ordered_peaks)
    _write_json(os.path.join(TMP_DIR, f"{data_id}_final_hits.json"), _df_to_records_json_safe(final_hits_df))

    if want_download:
        csv_bytes = _df_to_csv_bytes(final_hits_df)
        with open(os.path.join(TMP_DIR, f"{data_id}_final_hits.csv"), "wb") as f:
            f.write(csv_bytes)

    _progress(100)
    return {"data_id": data_id, "ok": True}

def plot_overview_compare_task(
    session_id: str,
    window: int,
    per_motif_pvals_json: str,
    want_download: bool,
    ...
    ids    = [ds.data_id for ds in datasets]

    motiflist_fp = os.path.join(TMP_DIR, f"{session_id}_motif_list.pkl")
    if not os.path.exists(motiflist_fp):
        raise RuntimeError(f"No cached motif_list for session {session_id}")
    motiflist = pd.read_pickle(motiflist_fp)

    per_motif_pvals = _parse_per_motif_pvals(per_motif_pvals_json)
    dfs = {}
    n = max(1, len(ids))

    for i, (label, data_id) in enumerate(zip(labels, ids), start=1):
        peaks_df = pd.read_pickle(os.path.join(TMP_DIR, f"{data_id}_peaks.pkl"))
        df_hits  = pd.read_pickle(os.path.join(TMP_DIR, f"{data_id}_df_hits.pkl"))
        ...
        _write_json(os.path.join(TMP_DIR, f"{session_id}_{label}_overview.json"), json.loads(fig.to_json()))
        _write_json(os.path.join(TMP_DIR, f"{session_id}_{label}_ordered_peaks.json"), ordered_peaks)
        _write_json(os.path.join(TMP_DIR, f"{session_id}_{label}_final_hits.json"), _df_to_records_json_safe(final_hits_df))

        dfs[label] = final_hits_df

        _progress(min(99, int(i / n * 100)))

    # optional download bundle(s)
    if want_download:
        if merge_files:
            merged = []
            ...
            with open(os.path.join(TMP_DIR, f"{session_id}_final_hits_merged.csv"), "wb") as f:
                f.write(csv_bytes)
        else:
            # zip individual CSVs
            zip_path = os.path.join(TMP_DIR, f"{session_id}_final_hits.zip")
            with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as z:
                for label, df in dfs.items():
                    csv_bytes = _df_to_csv_bytes(df)
                    z.writestr(f"{label}_final_hits.csv", csv_bytes)

    _progress(100)
    return {"session_id": session_id, "labels": labels, "ok": True}
